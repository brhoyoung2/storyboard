...
EMOTION_KEYWORDS = {
    ...
    "sharp":      ["날카로운", "날카롭게", "매서운", "매섭게", "예리", "사나운", "쏘아보"],
    "cold":       ["차가운", "차갑게", "냉정", "냉담", "싸늘", "무뚝뚝", "쌀쌀맞", "무심한"],
    "serious":    ["진지", "결연", "똑바로"],
...
POSE_KEYWORDS = [
    ...
    # 회전 표현(넘어짐/눕기/골아떨어짐) — 절(bow)·걷기 등 일반 포즈보다 먼저 매칭
    ("faint",       ["기절", "실신", "까무러", "혼절", "정신을 잃"]),
    ("sleep",       ["골아떨어", "곯아떨어", "잠들", "잠이 들", "쿨쿨", "곯아", "드러누워 자"]),
    ("lie",         ["눕는다", "누워", "드러눕", "눕다", "드러누"]),
    # 엄선 확장 포즈 — 구체 키워드를 일반 포즈(가리키/걷기 등)보다 먼저 매칭
    ("cover_mouth", ["입을 가리", "입을 막", "입을 틀어막", "입을 가린"]),
    ("wipe_tears",  ["눈물을 닦", "눈물을 훔", "눈가를 훔", "흐느"]),
    ("whisper",     ["귓속말", "속닥", "귀에 대고", "소곤"]),
    ("listen",      ["귀를 기울", "귀에 손", "엿듣"]),
    ("salute",      ["경례", "거수"]),
    ("both_out",    ["양팔을 벌", "두 팔을 벌", "팔을 벌리", "환영하", "반겨", "맞이"]),
    ("flex",        ["알통", "근육을 자랑", "힘자랑", "이두박근"]),
    ("block",       ["막아선", "방어", "막아낸", "팔로 막", "가드"]),
    ("stop_hand",   ["멈춰", "손바닥을 내밀", "정지", "스톱", "그만!"]),
    ("point_up",    ["위를 가리", "하늘을 가리", "위쪽을 가리", "검지를 치켜"]),
    ("chin_both",   ["양손으로 턱", "두 손으로 턱", "턱을 받치"]),
    ("hands_back_head", ["뒤통수에", "뒷머리에 손", "깍지를 끼"]),
    ("hands_behind", ["뒷짐", "팔을 뒤로", "손을 뒤로"]),
    ("crossed_legs", ["짝다리", "다리를 꼬", "다리 꼬"]),
    ("kick",        ["발로 찬", "발로 차", "발차기", "발길질", "걷어차", "걷어찬", "차버린"]),
    ("lean",        ["기대어", "기댄다", "기대선", "벽에 기"]),
    ("present",     ["보여준다", "제시한", "내보인", "선보인"]),
    ("wave_both",   ["두 손을 흔들", "양손을 흔들", "양손 흔"]),
...
PROP_RULES = [
    ...
    ("bed",    ["침대", "눕는다", "누워"]),
    ("sofa",   ["소파"]),
    # 엄선 확장 소품 — 손에 들거나 인물 옆/테이블 위
    ("umbrella", ["우산", "양산"]),
    ("bag",      ["핸드백", "가방을 들", "숄더백", "장바구니", "쇼핑백"]),
    ("backpack", ["백팩", "배낭", "책가방"]),
    ("plant",    ["화분", "화초", "관엽", "식물을"]),
    ("guitar",   ["기타를", "기타 치", "어쿠스틱", "일렉기타"]),
    ("mic",      ["마이크", "마이크를 들"]),
    ("balloon",  ["풍선"]),
    ("gift",     ["선물상자", "선물 상자", "선물을 건네", "선물 들"]),
    ("flower",   ["꽃을", "꽃다발", "장미", "꽃 한"]),
    ("ball",     ["공을", "축구공", "농구공", "야구공", "공놀이"]),
    ("suitcase", ["캐리어", "여행 가방", "여행가방", "트렁크"]),
    ("hat",      ["모자를", "모자 쓰", "캡을", "비니"]),
    ("coffee",   ["테이크아웃", "아메리카노", "라떼", "커피를 들"]),
    ("books",    ["책 더미", "책더미", "책을 쌓", "책 무더기"]),
    ("cake",     ["케이크"]),
    ("camera",   ["카메라"]),
    ("bottle",   ["물병", "병을 들", "음료수병", "페트병"]),
    ("glasses",  ["안경을", "선글라스"]),
]
